...
import { getWeeklyCalendarForAI, getDailyCalendarForAI, addEventForAI } from '../commands/calendar.js';
...
export const tools = {
    ...
    start_pomodoro: () => {
        startPomodoro({});
        return { status: 'Pomodoro timer started in the background.' };
    },
    stop_pomodoro: () => {
        stopPomodoro();
        return { status: 'Pomodoro timer stopped.' };
    },
    create_calendar_event: addEventForAI,
};
...
export const toolDeclarations = [
    ...
    {
        name: 'start_pomodoro',
        description: 'Start a Pomodoro timer with 25-minute work sessions and 5-minute breaks. The timer runs in the background.',
        parameters: {
            type: 'OBJECT',
            properties: {},
            required: []
...
export async function runStartPomodoro() {
    startPomodoro();
    return { message: 'Pomodoro timer started' };
}

export async function runStopPomodoro() {
    stopPomodoro();
